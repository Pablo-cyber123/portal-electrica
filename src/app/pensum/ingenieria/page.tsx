...
const MOCK_PENSUM_INGENIERIA = [
  ...
  },
  {
    ...
    credits: 16,
    subjects: ["Cálculo Multivariado", "Control Digital", "Sistemas de Potencia I", "Alta Tensión", "Termodinámica"]
  },
  {
    semester: "Semestre VIII",
    credits: 17,
    subjects: ["Sistemas de Potencia II", "Calidad de Energía", "Mercados de Energía", "Formulación y Evaluación de Proyectos"]
  },
  {
    semester: "Semestre IX",
    credits: 15,
    subjects: ["Energías Renovables", "Automatización Industrial", "Optativa I", "Proyecto de Ingeniería I"]
  },
  {
    semester: "Semestre X",
    credits: 14,
    subjects: ["Optativa II", "Optativa III", "Ética Profesional", "Proyecto de Ingeniería II (Grado)"]
  }
];

export default function PensumIngenieria() {
  return (
    <main className="min-h-screen bg-oxfordGrey-50 pb-20 pt-24 font-sans text-oxfordGrey-900">
      ...
      <div className="bg-white border-b border-oxfordGrey-200 mt-[-24px] pt-32 pb-16 px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-utsGreen-50 text-utsGreen-800 font-bold text-xs uppercase tracking-wider mb-6 border border-utsGreen-200">
            <GraduationCap className="w-4 h-4" /> Nivel Profesional Universitario
          </div>
          <h1 className="text-4xl md:text-6xl font-black text-oxfordGrey-900 tracking-tighter mb-6 leading-tight max-w-4xl">
            Ingeniería <span className="text-utsGreen-800">Eléctrica</span>
          </h1>
          <div className="flex flex-wrap gap-6 text-oxfordGrey-500 text-sm font-medium">
            <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-utsGreen-500" /> Duración: 10 Semestres (Ciclos Propedéuticos)</div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {MOCK_PENSUM_INGENIERIA.map((sem, idx) => (
            <div key={idx} className="bg-white rounded-2xl p-6 shadow-sm border border-oxfordGrey-200 hover:shadow-lg transition-shadow group relative overflow-hidden">
              {idx >= 6 && (
                // Destacar los semestres del ciclo profesional (7 al 10)
                <div className="absolute top-0 right-0 px-3 py-0.5 bg-utsGreen-50 text-utsGreen-800 text-[10px] font-bold uppercase rounded-bl-lg border-b border-l border-utsGreen-200">
                  Ciclo Prof.
                </div>
              )}
              <div className={`absolute top-0 left-0 w-1 h-full bg-gradient-to-b opacity-0 group-hover:opacity-100 transition-opacity ${idx >= 6 ? 'from-utsGreen-500 to-utsGreen-700' : 'from-oxfordGrey-300 to-oxfordGrey-400'}`} />
              
              <div className="flex items-center justify-between mb-6">
                <h3 className={`text-xl font-black uppercase tracking-tight ${idx >= 6 ? 'text-utsGreen-800' : 'text-oxfordGrey-800'}`}>{sem.semester}</h3>
              ...
              <ul className="space-y-3">
                {sem.subjects.map((subj, sIdx) => (
                  <li key={sIdx} className="flex items-start gap-3 text-sm text-oxfordGrey-700">
                    <div className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${idx >= 6 ? 'bg-utsGreen-500' : 'bg-oxfordGrey-300'}`} />
                    <span>{subj}</span>
                  </li>
                ))}
              </ul>
